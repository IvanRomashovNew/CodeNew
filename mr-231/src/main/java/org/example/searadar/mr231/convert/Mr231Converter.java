...
/**
 * Класс Mr231Converter является частью системы, которая работает с типом станции "МР-231".
 * <p>
 * Класс реализует интерфейс SearadarExchangeConverter и отвечает за
 * преобразование необработанных сообщений в список объектов SearadarStationMessage(TrackedTargetMessage,WaterSpeedHeadingMessage,RadarSystemDataMessage).
 * <p>
 * Конвертер поддерживает три типа сообщений:
 * - TTM (формуляр цели) - объект типа TrackedTargetMessage;
 * - VHW (курс и скорость относительно воды) - объект типа WaterSpeedHeadingMessage;
